...
<div class="col-lg-12 np" data-entity="form-entity" entity-name="room" entity-id-field="idroom">
    <!-- 
    ...
    <div class="col-lg-4 col-md-4 col-xs-12 pd1">
        ...
        <input field="data" name-field="title" type-field="input" class="form-control" id="title" placeholder="102" type="text" name="" value="{{$room->title}}"> 
    </div>
    <div class="col-lg-4"> 
        <p>Hotel</p>
        <select field="data" name-field="hotel_idhotel" type-field="input" class="form-control" id="planta" placeholder="" type="text" class="form-control"> 
            @foreach( $hotels as $k => $hotel )
                @if( $hotel->idhotel == $room->hotel_idhotel )
                    <option value="{{$hotel->idhotel}}" selected="selected">{{$hotel->name}}</option>
                @else  
    ...
    <div class="col-lg-4 col-md-4 col-xs-12 pd1">
        <p>Planta</p>
        <input field="data" name-field="planta" type-field="input" class="form-control" id="planta" placeholder="" type="text" name="" value="{{$room->planta}}">
    </div>
    <div class="col-lg-4 col-md-4 col-xs-12 pd1">
        <p>Tipo de habitación {{$room->type_room_idtype_room}}</p>
        <select field="data" name-field="type_room_idtype_room" type-field="input" class="form-control" id="planta" placeholder="" type="text" class="form-control">  
            @foreach( $types as $k => $type )
                @if( $type->idtype_room == $room->type_room_idtype_room)
                    <option value="{{$type->idtype_room}}" selected="selected">{{$type->title}} ({{$type->price_per_night}})</option>
                @else
    ...
    <div class="col-lg-4 col-md-4 col-xs-12 pd1">
        <p>Sección</p>
        <input field="data" name-field="section" type-field="input" class="form-control" id="type" placeholder="" type="text" name="" value="{{$room->section}}"> 
    </div>
    <div class="col-lg-12 pd1">
